...
measurements = []
for state in truth:
    ebr_measurement_no_noise = ebr_measurement_model.function(state, noise=False)  # make sure we only add noise if it's outside FOV, not just remove entirely...
    if not laser_receiver.is_outside_fov(ebr_measurement_no_noise[0]):
        # only add the measurement if we can "see" the laser in the FOV
